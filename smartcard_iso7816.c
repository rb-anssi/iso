...
int main(int argc, char* argv[]) {
	SC_ATR card;
	uint8_t T_protocol;
        if(SC_iso7816_fsm_init(&card, &T_protocol, 1, 1, 2, 64)){
               if(SC_iso7816_fsm_init(&card, &T_protocol, 0, 0, 0, 0)){
                       goto err;
               }
        }
        SC_APDU_cmd apdu;
        SC_APDU_resp resp;
        apdu.cla = 0x00; apdu.ins = 0xAA; apdu.p1 = 0x00; apdu.p2 = 0x00;
        apdu.lc = 200; apdu.le = 200; apdu.send_le = 1;
        if(SC_iso7816_send_APDU(&apdu, &resp, &card, T_protocol)) {
            goto err;
        }
